# Feasibilty Analysis of Air Robot in different weather and environmental conditions
...
* **Identification of Themes and Gaps**
    ...
    * Wind Effects: Wind speed and turbulence are consistently identified as significant factors affecting drone stability, control, and battery consumption.
    * Temperature Sensitivity: Extreme temperatures (both high and low) can adversely affect drone battery performance and overall system reliability.
    * Precipitation Risks: Rain, snow, and ice can compromise drone visibility, sensor accuracy, and aerodynamic performance.
    * Operational Limitations: Weather conditions impose limitations on drone flight time, range, and payload capacity.
    * Need for Advanced Forecasting: Accurate weather forecasting and real-time monitoring are crucial for safe and efficient drone operations.

    Gaps in the literature include:
    * Limited Research on Icing Effects: The impact of icing on drone performance and safety requires further investigation, especially in colder climates.
    * Lack of Standardized Testing: Standardized testing protocols are needed to evaluate drone performance under different weather conditions and to establish clear operational guidelines.
    * Integration of Weather Data: More research is needed on how to effectively integrate weather data into drone flight planning and control systems.
* **Objective**
...
* **Method**
    ...
    1.  Environment Setup
        * Defined a 3D simulation space with boundaries.
        * Generated random static and dynamic obstacles of varying shapes (cube, sphere, cylinder).
    2.  Path Planning Algorithm
        * Implemented the A\* algorithm to compute the optimal path from the start position to the destination.
        * Recalculated paths dynamically when obstacles were detected along the route.
    3.  Weather Integration
        * Retrieved real-time weather data from the WeatherBit API.
        * Monitored key parameters such as wind speed, visibility, precipitation, and temperature.
        * Assessed flight safety and aborted missions if weather conditions were unfavourable.
    4.  Visualization
        * Used MATLAB’s graphical tools to visualize drone movement, obstacles, and planned paths.
        * Represented the drone as a cube and plotted its real-time trajectory.
    5.  Dynamic Obstacles Handling
        * Introduced moving obstacles that changed positions during the simulation.
        * Periodically updated obstacle locations and recalculated paths to avoid collisions.
    6.  Emergency Response Mechanism
        * Implemented an emergency landing procedure if unsafe weather conditions were detected mid-flight.
        * Generated a safe descent path and simulated drone landing at the nearest viable position.
* **Key findings**
